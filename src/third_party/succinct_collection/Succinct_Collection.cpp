...
string Succinct_Collection::find_query(vector<pair<string, string>>& attr_val_vec) {
    if (!succinct_fd) {
        cout << "Succinct build unfinished!" << endl;
        return "";
    }
    set<int64_t> query_keys = find_key(attr_val_vec);
    vector<json> json_vec;
    auto succinct_ptr = (SuccinctShard*)succinct_fd;
    for (auto key : query_keys) {
        string doc_string;
        succinct_ptr->Get(doc_string, key);

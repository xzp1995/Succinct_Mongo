...
#include "mongo/rapidjson/document.h"
#include "mongo/rapidjson/writer.h"
#include "mongo/rapidjson/stringbuffer.h"
...
void generateLegacyQueryErrorResponse(const AssertionException& exception,
                                      ...
                                      CurOp* curop,
                                      Message* response) {
    //std::cout << "######################## generateLegacyQueryErrorResponse" << std::endl;
...
StatusWith<repl::ReadConcernArgs> _extractReadConcern(const CommandInvocation* invocation,
    ...
    //std::cout << "######################## _extractReadConcern" << std::endl;
...
bool runCommandImpl(OperationContext* opCtx,
                    ...
                    const OperationSessionInfoFromClient& sessionOptions) {
//    std::cout << "######################## runCommandImpl" << std::endl;
...
void execCommandDatabase(OperationContext* opCtx,
                         ...
                         rpc::ReplyBuilderInterface* replyBuilder,
                         const ServiceEntryPointCommon::Hooks& behaviors) {
//    std::cout << "######################## execCommandDatabase" << std::endl;
...
//            std::cout << "######################## execCommand: " << request.getCommandName() << std::endl;
        }
...
void curOpCommandSetup(OperationContext* opCtx, const OpMsgRequest& request) {
    //std::cout << "######################## curOpCommandSetup" << std::endl;
...
std::string getFilterValue(std::string req_str) {
    std::string res;
    size_t filter_pos = req_str.find("filter");
    if (filter_pos != std::string::npos) {
        size_t i = filter_pos + 8;
...
//    *resp --> OPMsgReply;
    ...
    std::cout << "---------------------response: " << resp_body.toString() << std::endl;
...
//{ cursor: { firstBatch: [ { _id: ObjectId('5bf22a19bdedbc0651f3aaec'), y: 1.0 } ], id: 0, ns: "test.test" }, ok: 1.0 }
        BufBuilder b;
        std::string ns("{ cursor: { firstBatch: [ { _id: ObjectId('5bf22a19bdedbc0651f3aaec'), y: 1.0 } ], id: 0, ns: \"test.test\" }, ok: 1.0 }");
...
//                              << "ok"
//                              << 1));
//        CursorResponse response_s = std::move(result.getValue());
//        BSONObj response_bson = response_s.toBSON(CursorResponse::ResponseType::InitialResponse);;

        //std::string response_str = response_bson.toString();
        //std::cout << "resp_bson: " << response_str << std::endl;

//        BSONObjBuilder builder;
//        ViewResponseFormatter f(response_bson);
//        f.appendAsDistinctResponse(&builder);
//        BSONObj final_response = builder.obj;
//        std::cout << "resp: " << final_response.toString() << std::endl;

//        Message toSend(std::move(final_response));
        Message toSend;
//        std::cout << "flag1" << std::endl;
        toSend.setData(1, b.buf(), b.len());
//        auto replyBuilder_s = rpc::makeReplyBuilder(rpc::protocolForMessage(toSend));
//        auto response_s = replyBuilder_s->done();
//        CurOp::get(opCtx)->debug().responseLength = response_s.header().dataLen();
        Message resp_msg_s = toSend;
//        std::cout << "flag2" << std::endl;
        auto resp_s = rpc::makeReply(&resp_msg_s);
//        std::cout << "flag3" << std::endl;
        BSONObj resp_body_s = resp_s->getCommandReply();
//        std::cout << "flag4" << std::endl;
//
        std::cout << "---------------------Succinctresponse: " << resp_body_s.toString() << std::endl;

        return DbResponse{std::move(toSend)};
    }
...
DbResponse receivedQuery(OperationContext* opCtx,
                         ...
                         const Message& m,
                         const ServiceEntryPointCommon::Hooks& behaviors) {
    //std::cout << "######################## receivedQuery" << std::endl;
...
void receivedKillCursors(OperationContext* opCtx, const Message& m) {
    //std::cout << "######################## receivedKillCursors" << std::endl;
    LastError::get(opCtx->getClient()).disable();
...
void receivedInsert(OperationContext* opCtx, const NamespaceString& nsString, const Message& m) {
    //std::cout << "######################## receivedInsert" << std::endl;
...
void receivedUpdate(OperationContext* opCtx, const NamespaceString& nsString, const Message& m) {
    //std::cout << "######################## receivedUpdate" << std::endl;
...
void receivedDelete(OperationContext* opCtx, const NamespaceString& nsString, const Message& m) {
    //std::cout << "######################## receivedDelete" << std::endl;
...
DbResponse receivedGetMore(OperationContext* opCtx,
    ...
    //std::cout << "######################## receivedGetMore" << std::endl;
